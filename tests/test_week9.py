...
df = pd.read_csv(
    "data/processed/compliance_query_dataset.csv",
    keep_default_na=False,
)

dataset = df.to_dict(orient="records")

report = evaluate(ComplianceChecker(), dataset)

print(f"Evaluated listings: {report['total']}")

for name, value in report["metrics"].items():
    print(f"{name}: {value:.1%}")

print("\nConfusion matrix:")
for name, value in report["confusion_matrix"].items():
    print(f"{name}: {value}")
